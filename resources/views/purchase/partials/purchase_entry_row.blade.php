    ...
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        @if(session('business.enable_lot_number'))
            @php
                ...
                $battery_percentage = !empty($imported_data['battery_percentage']) ? $imported_data['battery_percentage'] : null;
            @endphp
            <td>
                {!! Form::text('purchases[' . $row_count . '][lot_number]', $lot_number, ['class' => 'form-control input-sm', 'placeholder' => __('lang_v1.lot_number')]); !!}
            </td>
            <td>
                {!! Form::number('purchases[' . $row_count . '][battery_percentage]', $battery_percentage, ['class' => 'form-control input-sm', 'placeholder' => 'Battery %', 'min' => '0', 'max' => '100', 'step' => '0.01']); !!}
            </td>
        @endif
