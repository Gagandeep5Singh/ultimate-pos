        ...
        <div class="col-sm-8 col-sm-offset-2">
          <div class="form-group">
            {!! Form::label('location_id', __('business.business_location') . ':*') !!}
              {!! Form::select('location_id', $business_locations, isset($location_id) ? $location_id : null, ['class' => 'form-control select2',
              'placeholder' => __('lang_v1.select_location'), 'required', 'id' => 'location_id_select']); !!}
              <small class="help-block">@lang('lang_v1.select_location')</small>
          </div>
        </div>
        @else
          {!! Form::hidden('location_id', isset($location_id) ? $location_id : array_key_first($business_locations->toArray()) ); !!}
        @endif
        <div class="col-sm-8 col-sm-offset-2">
...
<script type="text/javascript">
    $(document).ready(function() {
        // Ensure location_id is selected before form submission
        $('#add_cash_register_form').on('submit', function(e) {
            var location_id = $('#location_id_select').val();
