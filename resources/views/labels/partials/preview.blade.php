...
@foreach($product_details as $details)
	@while($details['qty'] > 0)
		...
		<div style="display:inline-block;vertical-align:middle;line-height:16px !important;">
			...
			{{-- Barcode --}}
			@php
				$barcode_code = !empty($details['details']->sub_sku) ? $details['details']->sub_sku : $details['details']->sku;
				$barcode_code = trim((string)$barcode_code);
				$barcode_base64 = '';
				
				if (!empty($barcode_code)) {
					try {
						// Try larger size first (4, 120) for better scannability
						$barcode_base64 = DNS1D::getBarcodePNG($barcode_code, 'C128', 4, 120, array(0, 0, 0), true);
						
						// If that fails or returns empty, try smaller size (like receipts use)
						if (empty($barcode_base64) || strlen($barcode_base64) < 50) {
							$barcode_base64 = DNS1D::getBarcodePNG($barcode_code, 'C128', 2, 30, array(39, 48, 54), true);
						}
					} catch (\Exception $e) {
						// Fallback to smaller size on exception
						try {
							$barcode_base64 = DNS1D::getBarcodePNG($barcode_code, 'C128', 2, 30, array(39, 48, 54), true);
						} catch (\Exception $e2) {
							\Log::error('Barcode generation failed: ' . $e2->getMessage() . ' | Code: ' . $barcode_code);
						}
					}
				}
			@endphp
			@if(!empty($barcode_code) && !empty($barcode_base64) && strlen($barcode_base64) > 50)
				<img class="center-block" style="max-width:95% !important;height: {{$barcode_details->height*0.35}}in !important; display: block; margin: 0 auto;" src="data:image/png;base64,{{$barcode_base64}}" alt="Barcode: {{$barcode_code}}">
			@else
				<div style="text-align: center; font-size: 10px; color: red; padding: 5px;">Barcode Error: {{$barcode_code ?: 'NO CODE'}}</div>
			@endif
