		...
		<td align="center" valign="center">
			<div style="overflow: hidden !important;display: flex; flex-wrap: wrap;align-content: center;width: {{$barcode_details->width * 1}}in; height: {{$barcode_details->height * 1}}in; justify-content: center;">
				...
				<div>
					...
					{{-- Barcode --}}
					@php
						$barcode_code = !empty($page_product->sub_sku) ? $page_product->sub_sku : (isset($page_product->sku) ? $page_product->sku : '');
						...
						if (!empty($barcode_code)) {
							try {
								...
								$barcode_base64 = DNS1D::getBarcodePNG($barcode_code, 'C128', 4, 120, array(0, 0, 0), false);
					...
					@if(!empty($barcode_code) && !empty($barcode_base64) && strlen($barcode_base64) > 50)
						<img style="max-width:95% !important;height: {{$barcode_details->height*0.35}}in !important; display: block; margin: 0 auto;" src="data:image/png;base64,{{$barcode_base64}}" alt="Barcode: {{$barcode_code}}">
					@else
