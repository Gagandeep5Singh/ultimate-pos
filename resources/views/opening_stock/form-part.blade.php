...
@foreach($purchases[$key][$variation->id] as $sub_key => $var)
	...
	$battery_percentage = $var['battery_percentage'] ?? null;
...
@if($enable_lot == 1)
	<td>
		{!! Form::text('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][lot_number]', $lot_number , ['class' => 'form-control input-sm', 'placeholder' => __('lang_v1.lot_number')]); !!}
	</td>
	<td>
		{!! Form::number('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][battery_percentage]', $battery_percentage , ['class' => 'form-control input-sm', 'placeholder' => 'Battery %', 'min' => '0', 'max' => '100', 'step' => '0.01']); !!}
	</td>
@endif
	<td>
