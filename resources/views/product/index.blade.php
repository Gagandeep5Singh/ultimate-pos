...
@section('content')
                ...
                @component('components.filters', ['title' => __('report.filters')])
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="form-group">
                            {!! Form::select(
                                'active_state',
                                ['all' => __('lang_v1.all'), 'active' => __('business.is_active'), 'inactive' => __('lang_v1.inactive')],
                                'active',
                                [
                                    'class' => 'form-control select2',
                                    'style' => 'width:100%',
    ...
    <div class="form-group">
        ...
        &nbsp;  <!-- add a small space between the two checkboxes                         -->
        <label style="margin-left: 10px;">
            {!! Form::checkbox('out_stock', 1, false, ['class' => 'input-icheck', 'id' => 'out_stock']) !!} 
            <strong>out_stock</strong>
        </label>
    </div>
</div>
                    @if ($is_woocommerce)
                                    ...
                                    <div class="col-md-12">
                                        <div class="pull-right" style="margin-right: 15px; margin-top: 5px;">
                                            <label style="font-weight: normal; margin-right: 10px;">
                                                <input type="checkbox" id="accurate_search_toggle" style="margin-right: 5px;">
                                                <span>@lang('lang_v1.accurate_search')</span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            var locationColumns = @json($location_list->map(function($l){ return ['id'=>$l->id,'name'=>$l->name]; }));
            function reloadSubCategories() {
                var cats = $('#product_list_filter_category_id').val();
                if (!cats || cats.length === 0) {
                    $('#product_list_filter_sub_category_id').html('').trigger('change');
                    return;
                }
                $.ajax({
                    method: 'POST',
                    ...
                    success: function(result) {
                        if (result) {
                            $('#product_list_filter_sub_category_id').html(result).trigger('change');
                        }
                    }
                });
            }
            ...
            product_table = $('#product_table').DataTable({
                ...
                stateLoadCallback: function (settings) {
                    ...
                    var state = localStorage.getItem('DataTables_product_table_' + window.location.pathname);
                    if (state) {
                        // If state exists, load it (user's saved preferences)
                        return JSON.parse(state);
                    }
                    // If no state exists, return null to use defaults
                    return null;
                },
                aaSorting: [
                    [3, 'asc']
                ...
                "ajax": {
                    ...
                    "data": function(d) {
                        ...
                        d.out_stock = $('#out_stock').is(':checked');
                        ...
                        // Add accurate search toggle state
                        d.accurate_search = $('#accurate_search_toggle').is(':checked') ? 1 : 0;
                ...
                columns: [{
                    ...
                    // dynamic per-location stock columns
                    ...locationColumns.map(function(loc) {
                        return {
                            data: 'loc_' + loc.id,
                            name: 'loc_' + loc.id,
                            searchable: false,
                    ...
                    {
                            data: 'profit_after_klarna',
                            name: 'profit_after_klarna',
                            searchable: false,
                            orderable: false,
                            visible: false,
                            render: function(data, type, row) {
            ...
            // Apply default column visibility (hide location columns) only if no saved state exists
            // Column index calculation: 4 (after checkbox(0), image(1), action(2), current_stock(3))
            var savedState = localStorage.getItem('DataTables_product_table_' + window.location.pathname);
            if (!savedState) {
                // No saved state - apply defaults: hide all location columns
                var baseColumnIndex = 4;
                for (var i = 0; i < locationColumns.length; i++) {
                    product_table.column(baseColumnIndex + i).visible(false);
                }
                // Save this as the initial state so future changes are remembered
                product_table.state.save();
            }
            // If savedState exists, it was already loaded by stateLoadCallback above
           ...
           $(document).on('ifChanged', '#not_for_selling, #woocommerce_enabled, #out_stock', function() {
    if ($("#product_list_tab").hasClass('active')) {
        product_table.ajax.reload();
    }
    if ($("#product_stock_report").hasClass('active')) {
        stock_report_table.ajax.reload();
            ...
            // Reload table when accurate search toggle changes
            $(document).on('change', '#accurate_search_toggle', function() {
                if ($("#product_list_tab").hasClass('active')) {
                    product_table.ajax.reload();
                }
            });

            // Dependent sub-category load when categories change
...
$(document).ready(function () {
    ...
    if (typeof product_table !== 'undefined') {
        product_table.on('preXhr.dt', function (e, settings, data) {
            data.stock_filter = $('#out_stock_toggle').is(':checked') ? 'out' : '';
        });
    }

    // Reload DataTable on checkbox change
    $(document).on('ifChanged', '#out_stock_toggle', function () {
        if ($('#product_list_tab').hasClass('active')) {
            product_table.ajax.reload();
        }
    });
