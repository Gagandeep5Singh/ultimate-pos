        ...
        <div class="col-md-12">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <li class="active">
                        <a href="#psr_detailed_tab" data-toggle="tab" aria-expanded="true" class="report-tab-link"><i class="fa fa-list" aria-hidden="true"></i> @lang('lang_v1.detailed')</a>
                    </li>
                    <li>
                        <a href="#psr_detailed_with_purchase_tab" data-toggle="tab" aria-expanded="true" class="report-tab-link"><i class="fa fa-list" aria-hidden="true"></i> @lang('lang_v1.detailed_with_purchase')</a>
                    </li>
                    <li>
                        <a href="#psr_grouped_tab" data-toggle="tab" aria-expanded="true" class="report-tab-link"><i class="fa fa-bars" aria-hidden="true"></i> @lang('lang_v1.grouped')</a>
                    </li>
                    <li>
                        <a href="#psr_by_cat_tab" data-toggle="tab" aria-expanded="true" class="report-tab-link"><i class="fa fa-bars" aria-hidden="true"></i> @lang('lang_v1.by_category')</a>
                    </li>
                    <li>
                        <a href="#psr_by_brand_tab" data-toggle="tab" aria-expanded="true" class="report-tab-link"><i class="fa fa-bars" aria-hidden="true"></i> @lang('lang_v1.by_brand')</a>
                    </li>
                    <li>
                        <a href="#psr_all_products_tab" data-toggle="tab" aria-expanded="true" class="report-tab-link" data-tab="all_products"><i class="fa fa-list" aria-hidden="true"></i> @lang('lang_v1.all_products')</a>
                    </li>
                </ul>
                <div class="tab-content">
                    ...
                    @include('report.partials.product_sell_report_all_products')
                </div>
            </div>
        </div>
    </div>
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $(document).ready( function() {
            $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
                ...
                if ( target == '#psr_by_cat_tab') {
                    ...
                    if(typeof product_sell_report_by_brand_datatable == 'undefined') {
                        @php
                            ...
                            if(isset($location_list)) {
                                foreach($location_list as $loc) {
                                    $locationColumns[] = ['id' => $loc->id, 'name' => $loc->name];
                                }
                            }
                        @endphp
                        var locationColumns = @json($locationColumns ?? []);
                        ...
                        locationColumns.forEach(function(loc) {
                            columns.push({
                                data: 'loc_' + loc.id,
                                name: 'loc_' + loc.id,
                                searchable: false,
                        ...
                        @can('view_purchase_price')
                            columns.push({ data: 'purchase_price', name: 'purchase_price', searchable: false, orderable: false });
                        @endcan
                        @can('access_default_selling_price')
                            columns.push({ data: 'selling_price', name: 'selling_price', searchable: false, orderable: false });
                            columns.push({ data: 'discount', name: 'discount', searchable: false, orderable: false });
                            columns.push({ data: 'sale_price', name: 'sale_price', searchable: false, orderable: false });
                        @endcan
                        columns.push({ data: 'category_name', name: 'category_name', searchable: false });
                        columns.push({ data: 'sub_category_name', name: 'sub_category_name', searchable: false });
                        columns.push({ data: 'brand_name', name: 'brand_name', searchable: false });
                        columns.push({ data: 'total_qty_sold', name: 'total_qty_sold', searchable: false, orderable: false });
                        columns.push({ data: 'subtotal', name: 'subtotal', searchable: false, orderable: false });
                    ...
                    } else if ( target == '#psr_all_products_tab') {
                        if(typeof product_sell_report_all_products_datatable == 'undefined') {
                            @php
                                $locationColumns = [];
                                if(isset($location_list)) {
                                    foreach($location_list as $loc) {
                            ...
                            var columns = [
                                { data: 'product', name: 'products.name', searchable: true },
                                { data: 'sku', name: 'products.sku', searchable: true },
                                { data: 'current_stock', name: 'current_stock', searchable: false, orderable: false },
                            ];
                            ...
                            columns.push({ data: 'product_locations', name: 'product_locations', searchable: false });
                            ...
                            @can('access_default_selling_price')
                                ...
                                columns.push({ data: 'discount', name: 'discount', searchable: false, orderable: false });
                                columns.push({ data: 'sale_price', name: 'sale_price', searchable: false, orderable: false });
                                columns.push({ data: 'three_x_price', name: 'three_x_price', searchable: false, orderable: false });
                                columns.push({ data: 'normal_profit', name: 'normal_profit', searchable: false, orderable: false });
                                columns.push({ data: 'klarna_profit', name: 'klarna_profit', searchable: false, orderable: false });
                            @endcan
                            columns.push({ data: 'category', name: 'category', searchable: false });
                            columns.push({ data: 'sub_category', name: 'sub_category', searchable: false });
                            columns.push({ data: 'brand', name: 'brands.name', searchable: false });
                            columns.push({ data: 'tax', name: 'tax_rates.name', searchable: false });
                            
                            product_sell_report_all_products_datatable = $('table#product_sell_report_all_products').DataTable({
                                processing: true,
                                serverSide: true,
                                fixedHeader: false,
                                stateSave: true,
                                ...
                                order: [[0, 'asc']],
                                fnDrawCallback: function(oSettings) {
                                    $('#footer_psr_all_products_total_stock').html(
                                        __sum_stock($('#product_sell_report_all_products'), 'current_stock')
                                    );
                                    __currency_convert_recursively($('#product_sell_report_all_products'));
                                },
                            });
                        } else {
                            product_sell_report_all_products_datatable.ajax.reload();
                        }
                    }
                });
            });
            ...
            $('#export_all_products_btn').on('click', function() {
                var category_id = $('select#psr_filter_category_id').val();
                var brand_id = $('select#psr_filter_brand_id').val();
                var location_id = $('select#location_id').val();
                
                var url = '{{ action([\App\Http\Controllers\ReportController::class, "exportAllProductsReport"]) }}?';
