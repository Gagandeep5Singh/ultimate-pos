        ...
        <tr>
            ...
            @if( session()->get('business.enable_lot_number') == 1 && empty($for_ledger))
                <td>{{ $sell_line->lot_details->lot_number ?? '--' }}
                    ...
                    @endif
                    @if(!empty($sell_line->lot_details->battery_percentage) && $sell_line->lot_details->battery_percentage > 0)
                        @php
                            $battery = (float)$sell_line->lot_details->battery_percentage;
                            $battery_display = ($battery == floor($battery)) ? (int)$battery . '%' : number_format($battery, 2) . '%';
                        @endphp
