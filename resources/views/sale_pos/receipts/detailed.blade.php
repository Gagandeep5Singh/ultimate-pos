        ...
        <tr>
            <td>
                    ...
                    <div class="col-md-6 invoice-col width-50">
                        ...
                        @if (!empty($receipt_details->total_paid))
                            @php
                                ...
                                $total_paid_value = $receipt_details->total_paid;
                                $currency_symbol = $receipt_details->currency_symbol ?? '€';
                                
                                // Check if value contains negative sign
                                if (strpos($total_paid_value, '-') !== false || strpos($total_paid_value, '−') !== false) {
                                    $total_paid_value = '0,00 ' . $currency_symbol;
                                } else {
                                    // Also check if the numeric value is negative
                                    $clean_value = preg_replace('/[^\d,.-]/', '', $total_paid_value);
                        ...
                        @if (empty($receipt_details->letter_head))
                            ...
                            @if (!empty($receipt_details->logo))
                                <div style="margin-bottom: 20px; padding: 20px 0;">
                                    <img style="max-height: 200px; width: auto; display: block; margin: 0 auto;" src="{{ $receipt_details->logo }}"
                                        class="img center-block">
                                </div>
                                <!-- Double dashed separator after logo -->
                                <div style="margin: 20px 0; padding: 15px 0;">
                                    <hr style="border: none; border-top: 3px dashed #000; margin: 5px 0;">
                                    <hr style="border: none; border-top: 3px dashed #000; margin: 5px 0;">
                                </div>
                            @endif
                ...
                <div class="row ">
                    <div class="col-xs-12">
                        <br />
                        <table class="table table-no-top-cell-border table-slim" style="border-collapse: collapse; border: 2px dashed #000;">
                            <thead>
                                <tr style="background-color: #357ca5 !important; color: white !important; font-size: 15px !important; border-bottom: 2px dashed #000;"
                                    class="table-no-side-cell-border table-no-top-cell-border text-center">
                                    <td
                                        style="background-color: #357ca5 !important; color: white !important;width: 3% !important; border: 1px dashed #000; padding: 8px;">
                                        #</td>
                                    ...
                                    @endif
                                    <td
                                        style="background-color: #357ca5 !important; color: white !important; width: {{ $p_width }}% !important; border: 1px dashed #000; padding: 8px;">
                                        {{ $receipt_details->table_product_label }}
                                    </td>

                                    @if ($receipt_details->show_cat_code == 1)
                                        <td
                                            style="background-color: #357ca5 !important; color: white !important; width: 10% !important; border: 1px dashed #000; padding: 8px;">
                                    ...
                                    <td
                                        style="background-color: #357ca5 !important; color: white !important;width: 8% !important; border: 1px dashed #000; padding: 8px;">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if (!empty($line['sub_sku']))
                                                ...
                                                @endif
                                                @if (!empty($line['battery_percentage']))
                                                    | Battery: {{ $line['battery_percentage'] }}
                                                @endif
                ...
                <!-- Double dashed separator before payment section (Mail Splitter) -->
                <div style="margin: 25px 0; padding: 20px 0;">
                    <hr style="border: none; border-top: 4px dashed #000; margin: 8px 0;">
                    <hr style="border: none; border-top: 4px dashed #000; margin: 8px 0;">
                </div>
                <div class="row invoice-info " style="page-break-inside: avoid !important">
                    <div class="col-md-6 invoice-col width-50">
                        <table class="table table-slim" style="border-collapse: collapse;">
                            <thead>
                                <tr style="border-bottom: 2px dashed #000;">
                                    <th style="padding: 12px; border-bottom: 2px dashed #000; font-weight: bold;">Payment Method</th>
                                    <th class="text-right" style="padding: 12px; border-bottom: 2px dashed #000; font-weight: bold;">Amount</th>
                                    <th class="text-right" style="padding: 12px; border-bottom: 2px dashed #000; font-weight: bold;">Date</th>
                                </tr>
                            </thead>
                            <tbody>
                            @if (!empty($receipt_details->payments))
                                @foreach ($receipt_details->payments as $payment)
                                    <tr style="border-bottom: 1px dashed #ccc;">
                                        ...
                                        <td class="text-right" style="padding: 10px;"><strong>{{ $payment['amount'] }}</strong></td>
                                        <td class="text-right" style="padding: 10px;">{{ $payment['date'] }}</td>
                                    </tr>
                                @endforeach
                            @endif
                            <!-- Change Return-->
                            @if (!empty($receipt_details->change_return))
                                <tr style="border-bottom: 1px dashed #ccc;">
                                    <td style="padding: 10px;">@lang('lang_v1.change_return'):</td>
                                    <td class="text-right" style="padding: 10px;"><strong>{{ $receipt_details->change_return['amount'] }}</strong></td>
                                    <td class="text-right" style="padding: 10px;">{{ $receipt_details->change_return['date'] }}</td>
                                </tr>
                            @endif
                            </tbody>
                            <tfoot>
                            ...
                            @if (!empty($receipt_details->total_paid))
                                ...
                                @endphp
                                <tr style="border-top: 3px dashed #000; border-bottom: 3px dashed #000;">
                                    <th style="padding: 12px; font-size: 16px; font-weight: bold;">
                                        {!! $receipt_details->total_paid_label !!}
                                    </th>
                                    <td colspan="2" class="text-right" style="padding: 12px; font-size: 18px; font-weight: bold;">
                                        {{ $total_paid_value }}
                                    </td>
                                </tr>
                            @endif
                ...
                <!-- Double dashed separator before tax summary -->
                <div class="col-md-12" style="margin: 25px 0; padding: 20px 0;">
                ...
                <div class="col-md-12">
                    @if (empty($receipt_details->hide_price) && !empty($receipt_details->tax_summary_label))
                        ...
                        @if (!empty($receipt_details->taxes))
                            <table class="table table-slim" style="border-collapse: collapse;">
                                <tr style="border-bottom: 2px dashed #000;">
                                    <th colspan="2" class="text-center" style="padding: 12px; border-bottom: 2px dashed #000; font-weight: bold;">{{ $receipt_details->tax_summary_label }}
                                    </th>
                                </tr>
                                @foreach ($receipt_details->taxes as $key => $val)
                                    <tr style="border-bottom: 1px dashed #ccc;">
                                        <td class="text-center" style="padding: 10px;"><b>{{ $key }}</b></td>
                                        <td class="text-center" style="padding: 10px;">{{ $val }}</td>
                                    </tr>
                                @endforeach
                            </table>
                        @endif
                ...
                @if (!empty($receipt_details->additional_notes))
                    <!-- Double dashed separator before notes -->
                    <div class="col-xs-12" style="margin: 25px 0; padding: 20px 0;">
                        <hr style="border: none; border-top: 3px dashed #000; margin: 8px 0;">
                        <hr style="border: none; border-top: 3px dashed #000; margin: 8px 0;">
                    </div>
                    <div class="row ">
                        <div class="col-xs-12" style="padding: 15px 0;">
                            <p style="margin: 10px 0;">{!! nl2br($receipt_details->additional_notes) !!}</p>
                        </div>
                    </div>
                @endif

                <!-- Double dashed separator before footer (Mail Splitter) -->
                <div style="margin: 25px 0; padding: 20px 0;">
                    <hr style="border: none; border-top: 4px dashed #000; margin: 8px 0;">
                    <hr style="border: none; border-top: 4px dashed #000; margin: 8px 0;">
                </div>
...
/* Apply dashed borders to all table cells */
table td, table th {
    border: 1px dashed #000 !important;
}
