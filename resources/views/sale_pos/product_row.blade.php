...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		@else
			{!! $product_name !!}
		@endif
		{{-- Product image removed as per requirement --}}
		...
		{{-- Copy button for product name, SKU, and lot/serial number --}}
		<button type="button" 
			class="btn btn-xs btn-default copy-product-info" 
			title="Copy Product Name, SKU & Lot/Serial Number"
			data-product-name="{{$product->product_name}}"
			data-product-sku="{{$product->sub_sku}}"
			...
			<select class="form-control lot_number input-sm" name="products[{{$row_count}}][lot_no_line_id]" @if(!empty($product->transaction_sell_lines_id)) disabled @endif>
				...
				@foreach($product->lot_numbers as $lot_number)
					@php
						...
						//preselected IMEI if product searched by IMEI
					...
					@php
						$battery_text = '';
						if (!empty($lot_number->battery_percentage) && $lot_number->battery_percentage > 0) {
							$battery = (float)$lot_number->battery_percentage;
							if ($battery == floor($battery)) {
								$battery_text = ' | Battery: ' . (int)$battery . '%';
							} else {
								$battery_text = ' | Battery: ' . number_format($battery, 2) . '%';
							}
						}
					@endphp
					<option value="{{$lot_number->purchase_line_id}}" data-qty_available="{{$lot_number->qty_available}}" data-msg-max="@lang('lang_v1.quantity_error_msg_in_lot', ['qty'=> $lot_number->qty_formated, 'unit' => $product->unit  ])" {{$selected}}>@if(!empty($lot_number->lot_number) && $lot_enabled == 1){{$lot_number->lot_number}} @endif @if($lot_enabled == 1 && $exp_enabled == 1) - @endif @if($exp_enabled == 1 && !empty($lot_number->exp_date)) @lang('product.exp_date'): {{@format_date($lot_number->exp_date)}} @endif {{$battery_text}} {{$expiry_text}}</option>
				@endforeach
			</select>
		@endif
	@endif
