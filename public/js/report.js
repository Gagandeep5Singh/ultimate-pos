$(document).ready(function() {
    ...
    { data: 'total_cheque_payment', name: 'total_cheque_payment', searchable: false, visible: false  },
    ...
    { data: 'total_advance_payment', name: 'total_advance_payment', searchable: false ,visible: false  },
    ...
    { 
        ...
        render: function(data, type, row) {
            // Use server-provided HTML if present
            if (row.diff_cash_card !== undefined && row.diff_cash_card !== null && row.diff_cash_card !== '') {
                return row.diff_cash_card;
            }

            // Fallback: compute client-side to avoid DataTables “unknown parameter” warnings
            var toNum = function(val) {
                var n = parseFloat(val);
                return isNaN(n) ? 0 : n;
            };

            var cash_system = toNum($(row.total_cash_payment).data('orig-value')) || toNum(row.total_cash_payment);
            var cash_manual = toNum(row.closing_amount);
            var card_system = toNum($(row.total_card_payment).data('orig-value')) || toNum(row.total_card_payment);
            var card_manual = toNum(row.total_card_payment_closed);

            var cash_diff = cash_system - cash_manual;
            var card_diff = card_system - card_manual;

            var cash_class = (Math.abs(cash_diff) <= 1) ? 'text-success' : 'text-danger';
            var card_class = (Math.abs(card_diff) <= 1) ? 'text-success' : 'text-danger';

            var cash_html = '<span class="' + cash_class + '">' + __currency_trans_from_en(cash_diff, true) + '</span>';
            var card_html = '<span class="' + card_class + '">' + __currency_trans_from_en(card_diff, true) + '</span>';
...
createdRow: function(row, data, dataIndex) {
    // Remark column now only shows closing note; no per-row styling needed.
},
        "footerCallback": function ( row, data, start, end, display ) {
    ...
    if ($('#register_report_date_range').length == 1) {
    var drp = $('#register_report_date_range').data('daterangepicker');
    drp.setStartDate(moment());
    drp.setEndDate(moment());
    var date_string = moment().format(moment_date_format) + ' ~ ' + moment().format(moment_date_format);
    $('#register_report_date_range').val(date_string);
    updateRegisterReport();
}
    ...
    lot_report = $('table#lot_report').DataTable({
        ...
        columns: [
            ...
            { data: 'battery_percentage', name: 'pl.battery_percentage' },
