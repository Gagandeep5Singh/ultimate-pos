...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'key_invoice_no' => ! empty($input['key_invoice_no']) ? $input['key_invoice_no'] : null,
            'ref_no' => '',
   ...
   /**
 * Calculate Klarna fees for a period
 *
 * @param int $business_id
 ...
 * @return float
 */
public function getKlarnaFees($business_id, $start_date, $end_date, $location_id = null, $user_id = null)
{
    $query = TransactionPayment::join('transactions as t', 'transaction_payments.transaction_id', '=', 't.id')
        ->where('t.business_id', $business_id)
        ->where('t.type', 'sell')
        ->where('t.status', 'final')
        ->where('transaction_payments.method', 'custom_pay_6') // Klarna
    ...
    // Get all Klarna transactions
    $klarna_transactions = $query->select(
        'transaction_payments.amount as transaction_amount',
        't.id as transaction_id'
    )->get();

    $total_klarna_fees = 0;

    foreach ($klarna_transactions as $transaction) {
        // Calculate per-transaction fees: 4.99% + €0.35 per transaction
        $commission_fee = $transaction->transaction_amount * 0.0499; // 4.99%
        $fixed_fee = 0.35; // €0.35 per transaction
        
        $total_klarna_fees += $commission_fee + $fixed_fee;
    }

    return $total_klarna_fees;
}
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            // FIX: Only show total_due if there's actually a due amount (not fully paid)
            // Hide "Total Due: 0.00" when transaction is fully paid
            if ($due > 0) {
                $output['total_due'] = $this->num_f($due, $show_currency, $business_details);
                $output['total_due_label'] = $il->total_due_label;
            } else {
                $output['total_due'] = null; // Hide when fully paid
                $output['total_due_label'] = null;
            }
            ...
            //Get payment details
            $output['payments'] = [];
            $output['change_return'] = null; // Store change return separately for display
            if ($il->show_payments == 1) {
                ...
                if (! empty($payments)) {
                    foreach ($payments as $value) {
                        // Include change return payments in invoice display
                        $method = ! empty($payment_types[$value['method']]) ? $payment_types[$value['method']] : '';
                        
                        // Store change return separately for better display
                        if ($transaction_type == 'sell' && isset($value['is_return']) && $value['is_return'] == 1) {
                            ...
                            continue; // Skip adding to payments list, we'll show it separately
                        }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            if ($is_lot_number_enabled == 1 && $il->show_lot == 1) {
                ...
                if (!empty($line->lot_details->battery_percentage) && $line->lot_details->battery_percentage > 0) {
                    $battery = (float)$line->lot_details->battery_percentage;
                    if ($battery == floor($battery)) {
                        $line_array['battery_percentage'] = (int)$battery . '%';
                    } else {
                        $line_array['battery_percentage'] = number_format($battery, 2) . '%';
                    }
                } else {
                    $line_array['battery_percentage'] = null;
                }
            }
    ...
    {
        // For sell transactions, only sum payments where is_return = 0
        // Refund payments (is_return = 1) should not be included in total_paid for the original sale
        $transaction = Transaction::find($transaction_id);
        
        if ($transaction && $transaction->type == 'sell') {
            // For sell transactions, exclude refund payments (is_return = 1)
            // Also ensure we only sum positive amounts (in case of data issues)
            $total_paid = TransactionPayment::where('transaction_id', $transaction_id)
                    ->where('is_return', 0)
                    ->where('amount', '>', 0)  // Only positive amounts
                    ->sum('amount');
            
            // FIX: Cap total_paid at final_total to avoid showing payment amount (20.00) instead of sale amount (9.99)
            // If customer paid more than sale amount, total_paid should be the sale amount, not the payment amount
    ...
    public function getLotNumbersFromVariation($variation_id, $business_id, $location_id, $exclude_empty_lot = false)
    {
        ...
        $purchase_lines = $query->select('purchase_lines.id as purchase_line_id', 'lot_number', 'purchase_lines.battery_percentage', 'purchase_lines.exp_date as exp_date', DB::raw('(purchase_lines.quantity - (purchase_lines.quantity_sold + purchase_lines.quantity_adjusted + purchase_lines.quantity_returned)) AS qty_available'))->get();
    ...
    public function getProfitLossDetails($business_id, $location_id, $start_date, $end_date, $user_id = null, $permitted_locations = null)
    {
        ...
        // Calculate net profit WITHOUT Klarna fees first
        ...
        $klarna_fees = $this->getKlarnaFees($business_id, $start_date, $end_date, $location_id, $user_id);
        
        // Store Klarna fees separately for display (do NOT add to total_expense)
        $data['total_klarna_fees'] = $klarna_fees;
        
        // Store net profit BEFORE Klarna fees
        $data['net_profit_before_klarna'] = $data['net_profit'];
        
        // Calculate net profit AFTER Klarna fees
        $data['net_profit'] = $data['net_profit'] - $klarna_fees;
    ...
    public function registerReport($business_id, $permitted_locations, $start_date = null, $end_date = null, $user_id = null)
{
    $registers = CashRegister::leftjoin(
        ...
        ->select(
            ...
            'bl.name as location_name',
            'cash_registers.closing_note as remark', // <-- THIS LINE FIXES THE ERROR
            // FIX: Calculate total cash: opening cash + cash sales - cash refunds - cash expenses
            // This shows the actual cash balance in the register
            DB::raw("SUM(IF(transaction_type='initial', amount, 0)) + SUM(IF(pay_method='cash', IF(transaction_type='sell', IF(ct.type='credit', amount, -1*amount), 0), 0)) - SUM(IF(pay_method='cash', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='cash', IF(transaction_type='expense', amount, 0), 0)) as total_cash_payment"),
            // Calculate cheque: sales - refunds - expenses
            DB::raw("SUM(IF(pay_method='cheque', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='cheque', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='cheque', IF(transaction_type='expense', amount, 0), 0)) as total_cheque_payment"),
            // Calculate card: sales - refunds - expenses
            DB::raw("SUM(IF(pay_method='card', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='card', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='card', IF(transaction_type='expense', amount, 0), 0)) as total_card_payment"),
            // Calculate bank transfer: sales - refunds - expenses
            DB::raw("SUM(IF(pay_method='bank_transfer', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='bank_transfer', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='bank_transfer', IF(transaction_type='expense', amount, 0), 0)) as total_bank_transfer_payment"),
            // Calculate other: sales - refunds - expenses
            DB::raw("SUM(IF(pay_method='other', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='other', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='other', IF(transaction_type='expense', amount, 0), 0)) as total_other_payment"),
            // Calculate advance: sales - refunds - expenses
            DB::raw("SUM(IF(pay_method='advance', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='advance', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='advance', IF(transaction_type='expense', amount, 0), 0)) as total_advance_payment"),
            // Calculate custom payment methods: sales - refunds - expenses
            DB::raw("SUM(IF(pay_method='custom_pay_1', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_1', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_1', IF(transaction_type='expense', amount, 0), 0)) as total_custom_pay_1"),
            DB::raw("SUM(IF(pay_method='custom_pay_2', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_2', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_2', IF(transaction_type='expense', amount, 0), 0)) as total_custom_pay_2"),
            DB::raw("SUM(IF(pay_method='custom_pay_3', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_3', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_3', IF(transaction_type='expense', amount, 0), 0)) as total_custom_pay_3"),
            DB::raw("SUM(IF(pay_method='custom_pay_4', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_4', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_4', IF(transaction_type='expense', amount, 0), 0)) as total_custom_pay_4"),
            DB::raw("SUM(IF(pay_method='custom_pay_5', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_5', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_5', IF(transaction_type='expense', amount, 0), 0)) as total_custom_pay_5"),
            DB::raw("SUM(IF(pay_method='custom_pay_6', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_6', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_6', IF(transaction_type='expense', amount, 0), 0)) as total_custom_pay_6"),
            DB::raw("SUM(IF(pay_method='custom_pay_7', IF(transaction_type='sell', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_7', IF(transaction_type='refund', amount, 0), 0)) - SUM(IF(pay_method='custom_pay_7', IF(transaction_type='expense', amount, 0), 0)) as total_custom_pay_7")
