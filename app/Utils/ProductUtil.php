...
class ProductUtil extends Util
{
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        $product = $query->select(
            ...
            'variations.sale_price as sale_price',
        ...
        // Auto-apply sale price as a line discount for POS/quick add
        if (!empty($product->sale_price)
            && $product->sale_price > 0
            && $product->sale_price < $product->sell_price_inc_tax) {

            $product->original_sell_price_inc_tax = $product->sell_price_inc_tax;
            $sale_discount = $product->sell_price_inc_tax - $product->sale_price;

            // Keep main price equal to sale price, record discount for UI/totals
            $product->sell_price_inc_tax = $product->sale_price;
            $product->line_discount_type = 'fixed';
            $product->line_discount_amount = $sale_discount;
        } else {
            $product->original_sell_price_inc_tax = $product->sell_price_inc_tax;
            $product->line_discount_type = null;
    ...
    public function calculateComboDetails($location_id, $combo_variations)
    {
        ...
        foreach ($combo_variations as $key => $value) {
         ...
         // --- AUTOMATIC SALE PRICE / DISCOUNT ---
    // If this variation has a sale_price lower than the normal price,
    // we keep the normal price and apply the difference as a FIXED discount.
    if (!empty($details->sale_price)
        && $details->sale_price > 0
        && $details->sale_price < $details->sell_price_inc_tax) {

        // Optional: remember original price if you want to show it somewhere
        $details->original_sell_price_inc_tax = $details->sell_price_inc_tax;

        // Discount per unit (regular - sale)
        $sale_discount = $details->sell_price_inc_tax - $details->sale_price;

        // These two are used by the POS line & edit popup
        $details->line_discount_type   = 'fixed';
        $details->line_discount_amount = $sale_discount;
    }
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            $purchase_line->battery_percentage = ! empty($data['battery_percentage']) ? $this->num_uf($data['battery_percentage']) : null;
