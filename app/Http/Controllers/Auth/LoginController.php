...
class LoginController extends Controller
{
    ...
    */
    ...
    protected function authenticated(Request $request, $user)
    {
        ...
        // Force user to select a location on every login before continuing
        $defaultRedirect = $this->redirectTo();
        $intended = $request->session()->pull('url.intended', $defaultRedirect);

        $request->session()->forget('user.location_id');
        $request->session()->put('post_login_redirect', $intended);

        return redirect()->route('login.select_location');
    }
