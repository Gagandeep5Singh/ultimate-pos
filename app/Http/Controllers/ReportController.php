    ...
    public function getProfitLoss(Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        // Klarna fees are already calculated in getProfitLossDetails method
        // Ensure values are set with defaults if not present
        $data['total_klarna_fees'] = $data['total_klarna_fees'] ?? 0;
        $data['net_profit_before_klarna'] = $data['net_profit_before_klarna'] ?? ($data['net_profit'] ?? 0) + ($data['total_klarna_fees'] ?? 0);
    ...
    return '<span data-orig-value="'.$row->total_card_payment.'" >'.$this->transactionUtil->num_f($row->total_card_payment, true).'</span>';
                ...
                ->addColumn('diff_cash_card', function ($row) {
                    $cash_system = $row->total_cash_payment ?? 0;
                    $cash_manual = $row->closing_amount ?? 0;
                    $card_system = $row->total_card_payment ?? 0;
                    $card_manual = $row->total_card_payment_closed ?? 0;

                    $cash_diff = $cash_system - $cash_manual;
                    $card_diff = $card_system - $card_manual;

                    $cash_class = (abs($cash_diff) <= 1) ? 'text-success' : 'text-danger';
                    $card_class = (abs($card_diff) <= 1) ? 'text-success' : 'text-danger';

                    $cash_html = '<span class="'.$cash_class.'">'.$this->transactionUtil->num_f($cash_diff, true).'</span>';
                    $card_html = '<span class="'.$card_class.'">'.$this->transactionUtil->num_f($card_diff, true).'</span>';

                    return 'Cash: '.$cash_html.'<br>Card: '.$card_html;
                })
                ...
                ->rawColumns(['action', 'user_name', 'total_card_payment', 'total_cheque_payment', 'total_cash_payment', 'total_bank_transfer_payment', 'total_other_payment', 'total_advance_payment', 'total_custom_pay_1', 'total_custom_pay_2', 'total_custom_pay_3', 'total_custom_pay_4', 'total_custom_pay_5', 'total_custom_pay_6', 'total_custom_pay_7', 'total', 'diff_cash_card', 'remark'])
                ->make(true);
        }
    ...
    public function getStockExpiryReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (! empty($request->input('sub_category_id'))) {
                $subcat = $request->input('sub_category_id');
                if (is_array($subcat)) {
                    $query->whereIn('p.sub_category_id', $subcat);
                } else {
                    $query->where('p.sub_category_id', $subcat);
                }
            }
            if (! empty($request->input('brand_id'))) {
                $brand = $request->input('brand_id');
                if (is_array($brand)) {
                    $query->whereIn('p.brand_id', $brand);
                } else {
            ...
            if (! empty($request->input('unit_id'))) {
                ...
                if (is_array($unit)) {
                    $query->whereIn('p.unit_id', $unit);
                } else {
                    $query->where('p.unit_id', $unit);
                }
            }
            ...
            $report = $query->select(
                ...
                DB::raw('MAX(t.ref_no) as ref_no'),
                DB::raw('MAX(t.id) as transaction_id'),
                DB::raw('MAX(purchase_lines.id) as purchase_line_id'),
                'purchase_lines.lot_number',
                DB::raw('COALESCE(MAX(purchase_lines.battery_percentage), 0) as battery_percentage')
            )
            ->having('stock_left', '>', 0)
            ->groupBy('purchase_lines.variation_id')
            ->groupBy('purchase_lines.exp_date')
            ->groupBy('purchase_lines.lot_number')
            ->groupBy('purchase_lines.battery_percentage');
            ...
            return Datatables::of($report)
                ...
                ->addColumn('battery_percentage', function ($row) {
                    // Get battery_percentage from the row, default to 0 if not set
                    $battery = 0;
                    if (isset($row->battery_percentage) && $row->battery_percentage !== null && $row->battery_percentage !== '') {
                        $battery = (float)$row->battery_percentage;
                    }
                    if ($battery > 0) {
                        // Show whole numbers without decimals (100% instead of 100.00%)
                        if ($battery == floor($battery)) {
                            return (int)$battery . '%';
                        } else {
                            return number_format($battery, 2) . '%';
                        }
                    } else {
                        return '--';
                    }
                ...
                ->addColumn('total_sold', function ($row) {
                    // Get total sold for this lot number
                    $purchase_line = \App\PurchaseLine::where('id', $row->purchase_line_id)->first();
                    if ($purchase_line) {
                        $total_sold = $purchase_line->quantity_sold ?? 0;
                        return '<span data-is_quantity="true" class="display_currency" data-currency_symbol=false data-orig-value="'.$total_sold.'">'.$total_sold.'</span> '.$row->unit;
                    }
                    return '<span data-is_quantity="true" class="display_currency" data-currency_symbol=false data-orig-value="0">0</span> '.$row->unit;
                })
                ->addColumn('total_adjusted', function ($row) {
                    // Get total adjusted for this lot number
                    $purchase_line = \App\PurchaseLine::where('id', $row->purchase_line_id)->first();
                    if ($purchase_line) {
                        $total_adjusted = $purchase_line->quantity_adjusted ?? 0;
                        return '<span data-is_quantity="true" class="display_currency" data-currency_symbol=false data-orig-value="'.$total_adjusted.'">'.$total_adjusted.'</span> '.$row->unit;
                    }
                    return '<span data-is_quantity="true" class="display_currency" data-currency_symbol=false data-orig-value="0">0</span> '.$row->unit;
                })
                ...
                ->rawColumns(['exp_date', 'ref_no', 'edit', 'stock_left', 'stock', 'total_sold', 'total_adjusted', 'battery_percentage'])
                ->make(true);
        }
    ...
    public function updateStockExpiryReport(Request $request)
    {
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                $input = $request->only(['purchase_line_id', 'exp_date', 'battery_percentage']);
                ...
                if (! empty($purchase_line)) {
                    if (! empty($input['exp_date'])) {
                    $purchase_line->exp_date = $this->productUtil->uf_date($input['exp_date']);
                    }
                    if (isset($input['battery_percentage'])) {
                        $purchase_line->battery_percentage = ! empty($input['battery_percentage']) ? $this->productUtil->num_uf($input['battery_percentage']) : null;
                    }
                    $purchase_line->save();
                }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        // Get permitted locations for location stock columns
        $permitted_locations = auth()->user()->permitted_locations();
        $location_list = $permitted_locations == 'all'
            ? BusinessLocation::where('business_id', $business_id)->get()
            : BusinessLocation::where('business_id', $business_id)->whereIn('id', $permitted_locations)->get();

        return view('report.product_sell_report')
            ->with(compact('business_locations', 'customers', 'categories', 'brands',
                'customer_group', 'product_custom_field1', 'product_custom_field2', 'location_list'));
    }

    /**
     * Get all products for report (without custom fields and image)
     *
     * @return \Illuminate\Http\Response
     */
    public function getAllProductsReport(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            // Get location IDs for columns
            $location_ids_for_columns = $permitted_locations == 'all'
                ? BusinessLocation::where('business_id', $business_id)->pluck('id')
                : collect((array)$permitted_locations);

            // Build current_stock subquery using raw SQL to avoid binding issues - now per variation
            ...
            // Per-location stock columns using raw SQL subqueries with embedded location IDs to avoid binding issues
            foreach ($location_ids_for_columns as $locId) {
                // Use DB::raw with embedded location ID (not as parameter) to avoid binding issues with count queries
        ...
        // Apply filters BEFORE groupBy
            ...
            if (! empty($category_id)) {
                if (is_array($category_id)) {
                    $products->whereIn('products.category_id', $category_id);
                } else {
            ...
            if (! empty($sub_category_id)) {
                if (is_array($sub_category_id)) {
                    $products->whereIn('products.sub_category_id', $sub_category_id);
                } else {
            ...
            if (! empty($brand_id)) {
                if (is_array($brand_id)) {
                    $products->whereIn('products.brand_id', $brand_id);
                } else {
            ...
            $location_id = $request->get('location_id', null);
            if ($location_id == 'none') {
                $products->doesntHave('product_locations');
            } elseif (! empty($location_id)) {
                if ($permitted_locations != 'all') {
                    $products->whereHas('product_locations', function ($query) use ($permitted_locations) {
                        $query->whereIn('product_locations.location_id', $permitted_locations);
                    });
                }
            }

            // Group by variation ID (like By Brand report) to show each variation as separate row
            ...
            $datatable = Datatables::of($products)
                ...
                ->addColumn('product', function ($row) {
                    if ($row->product_type == 'variable') {
                        $product = $row->product_name . ' - ' . ($row->product_variation ?? '') . ' - ' . ($row->variation_name ?? '');
                    } else {
                    ...
                    if ($row->not_for_selling == 1) {
                        $product .= ' <span class="label bg-gray">'.__('lang_v1.not_for_selling').'</span>';
                    }
                    return $product;
                })
                ...
                ->addColumn('purchase_price', function ($row) {
                    return $this->transactionUtil->num_f($row->purchase_price ?? 0, true);
                })
                ...
                ->addColumn('sale_price', function ($row) {
                    if (!empty($row->sale_price) && $row->sale_price > 0 && $row->sale_price < $row->selling_price) {
                        return '<span class="text-danger">' .
                            $this->productUtil->num_f($row->sale_price, true) .
                            '</span>';
                    }
                    return '';
                })
                ->addColumn('discount', function ($row) {
                    if (!empty($row->sale_price) && $row->sale_price > 0 && $row->sale_price < $row->selling_price) {
                        $diff = $row->selling_price - $row->sale_price;
                        $percent = round($diff / $row->selling_price * 100);
                        return $this->productUtil->num_f($diff, true) . ' (' . $percent . '%)';
                    }
                    return '';
                })
                ->addColumn('three_x_price', function ($row) {
                    $selling_price = (!empty($row->sale_price) && $row->sale_price > 0 && $row->sale_price < $row->selling_price)
                        ? $row->sale_price
                        : $row->selling_price;
                    return $this->productUtil->num_f($selling_price / 3, true);
                })
                ->addColumn('normal_profit', function ($row) {
                    $selling_price = (!empty($row->sale_price) && $row->sale_price > 0 && $row->sale_price < $row->selling_price)
                        ? $row->sale_price
                        : $row->selling_price;
                    $cost_price = $row->purchase_price ?? 0;
                    if ($selling_price > 0 && $cost_price > 0) {
                        return $this->productUtil->num_f($selling_price - $cost_price, true);
                    }
                    return '';
                })
                ->addColumn('klarna_profit', function ($row) {
                    $selling_price = (!empty($row->sale_price) && $row->sale_price > 0 && $row->sale_price < $row->selling_price)
                        ? $row->sale_price
                        : $row->selling_price;
                    $cost_price = $row->purchase_price ?? 0;
                    $normal_profit = $selling_price - $cost_price;

                    if ($selling_price > 0 && $normal_profit > 0) {
                        $klarna_fee = ($selling_price * 0.0499) + 0.35;
                        return $this->productUtil->num_f($normal_profit - $klarna_fee, true);
                    }
                    return '';
                });

            // Add location column renderers
            foreach ($location_ids_for_columns as $locId) {
                $datatable->addColumn('loc_' . $locId, function ($row) use ($locId) {
                    $colName = 'loc_' . $locId;
                    $data = $row->$colName ?? 0;
                    if (!$data || $data == 0) {
                        return '';
            ...
            $raw_columns = ['current_stock', 'category', 'product', 'product_locations', 'sale_price', 'discount', 'purchase_price', 'selling_price'];
            foreach ($location_ids_for_columns as $locId) {
                $raw_columns[] = 'loc_' . $locId;
            }
    ...
    /**
     * Export all products report to Excel
     */
    public function exportAllProductsReport(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }

        // Set maximum php execution time and memory for large exports
        ...
        // Build query same as getAllProductsReport - join with variations to show each variation as separate row
        $query = Product::join('variations as v', 'products.id', '=', 'v.product_id')
            ->leftjoin('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
            ->leftjoin('categories as c1', 'products.category_id', '=', 'c1.id')
            ->leftjoin('categories as c2', 'products.sub_category_id', '=', 'c2.id')
        ...
        // Select variation-level data (like By Brand report)
        $products = $query->select(
            'products.id as product_id',
            'products.name as product_name',
            'products.type as product_type',
        ...
        // Per-location stock columns using raw SQL with embedded location IDs to avoid binding issues
        foreach ($location_ids_for_columns as $locId) {
            // Use DB::raw with embedded location ID (not as parameter) to avoid binding issues with count queries
            $products->addSelect(DB::raw("(SELECT COALESCE(SUM(vld3.qty_available), 0) FROM variation_location_details as vld3 WHERE vld3.variation_id = v.id AND vld3.location_id = " . (int)$locId . ") as loc_{$locId}"));
        }

        // Apply filters BEFORE groupBy to avoid issues
        $category_id = $request->get('category_id', null);
        if (! empty($category_id)) {
            if (is_array($category_id)) {
                $products->whereIn('products.category_id', $category_id);
        ...
        if ($location_id == 'none') {
            ...
            $products->whereHas('product_locations', function ($query) use ($location_id) {
        ...
        $simple_count = Product::where('business_id', $business_id)
            ->where('type', '!=', 'modifier')
            ->count();
        \Log::info('Export All Products - Simple product count (no joins): ' . $simple_count);

        try {
            // Use count first to check if query is valid
            $product_count = $products->count();
            \Log::info('Export All Products - Query count before get: ' . $product_count);
            
            if ($product_count > 10000) {
                \Log::warning('Export All Products - Large dataset detected: ' . $product_count . ' products');
            }
            
            $products = $products->get();
        } catch (\Exception $e) {
            \Log::error('Export All Products Query Error: ' . $e->getMessage());
            \Log::error('Export All Products Query Trace: ' . $e->getTraceAsString());
            \Log::error('Export All Products Query SQL: ' . $products->toSql());
            \Log::error('Export All Products Query Bindings: ' . json_encode($products->getBindings()));
            return back()->with('error', 'Error fetching products: ' . $e->getMessage());
        }

        // Debug: Log product count
        \Log::info('Export All Products - Product count after query: ' . $products->count());
        \Log::info('Export All Products - Business ID: ' . $business_id);
        \Log::info('Export All Products - Permitted Locations: ' . (is_array($permitted_locations) ? implode(',', $permitted_locations) : $permitted_locations));
        \Log::info('Export All Products - Category filter: ' . $request->get('category_id', 'none'));
        \Log::info('Export All Products - Brand filter: ' . $request->get('brand_id', 'none'));
        \Log::info('Export All Products - Location filter: ' . ($location_id ?? 'none'));
        
        // If no products found, return early with message
        if ($products->isEmpty()) {
            \Log::warning('Export All Products - No products found, returning empty export');
            return back()->with('error', 'No products found matching the selected filters.');
        }
        ...
        foreach ($products as $product) {
            // Format product name like By Brand report
            ...
            $row = [
                $product_name,
                $sku,
                $product->enable_stock ? number_format($product->current_stock, 2) : '--',
            ];
            ...
            foreach ($location_ids_for_columns as $locId) {
                ...
                $row[] = $stock > 0 ? number_format($stock, 2) : '';
            }

            // Business locations
            $product_obj = Product::find($product->product_id);
            $product_locations = $product_obj ? $product_obj->product_locations : collect();
            $row[] = $product_locations->pluck('name')->implode(', ');

            // Purchase price
            if (auth()->user()->can('view_purchase_price')) {
                $row[] = $this->transactionUtil->num_f($product->purchase_price ?? 0, true);
            }

            // Selling price and related
            if (auth()->user()->can('access_default_selling_price')) {
                $row[] = $this->transactionUtil->num_f($product->selling_price ?? 0, true);

                // Discount
                $discount = '';
                if (!empty($product->sale_price) && $product->sale_price > 0 && $product->sale_price < $product->selling_price) {
                    $diff = $product->selling_price - $product->sale_price;
                    $percent = round($diff / $product->selling_price * 100);
                ...
                $sale_price = '';
                if (!empty($product->sale_price) && $product->sale_price > 0 && $product->sale_price < $product->selling_price) {
                    $sale_price = $this->transactionUtil->num_f($product->sale_price, true);
                }
                $row[] = $sale_price;

                // 3x price
                $selling_price_val = (!empty($product->sale_price) && $product->sale_price > 0 && $product->sale_price < $product->selling_price)
                    ? $product->sale_price
                    : $product->selling_price;
                $row[] = $this->transactionUtil->num_f($selling_price_val / 3, true);

                // Normal profit
                $cost_price = $product->purchase_price ?? 0;
                $normal_profit = '';
                if ($selling_price_val > 0 && $cost_price > 0) {
                    $normal_profit = $this->transactionUtil->num_f($selling_price_val - $cost_price, true);
                }
                $row[] = $normal_profit;
                ...
                $klarna_profit = '';
                if ($selling_price_val > 0 && $cost_price > 0) {
                    $normal_profit_val = $selling_price_val - $cost_price;
        ...
        // Debug: Log export data count
        \Log::info('Export All Products - Export data rows: ' . count($export_data));
        \Log::info('Export All Products - Headers count: ' . count($headers));
        
        // If no data, still return headers with a message row
        if (empty($export_data)) {
            $export_data = [
                array_fill(0, count($headers), 'No products found matching the selected filters.')
            ];
        }
        
        // Create Excel file
        $filename = 'all_products_report_' . Carbon::now()->format('Y-m-d_His') . '.xlsx';
        
        try {
            \Log::info('Export All Products - Starting Excel generation with ' . count($export_data) . ' rows and ' . count($headers) . ' columns');
            
            $export = new AllProductsReportExport($headers, $export_data);
            
            \Log::info('Export All Products - Excel object created, starting download');
            
            return Excel::download($export, $filename);
        } catch (\Exception $e) {
            \Log::error('Export All Products Excel Error: ' . $e->getMessage());
            \Log::error('Export All Products Excel Trace: ' . $e->getTraceAsString());
            \Log::error('Export All Products Excel File: ' . $e->getFile() . ' Line: ' . $e->getLine());
            return back()->with('error', 'Error exporting file: ' . $e->getMessage());
        }
    }

    /**
     * Update CSV file for Google Sheets import (saves to fixed location)
     *
     * @return \Illuminate\Http\Response
     */
    public function updateProductsCsvForGoogleSheets(Request $request)
    {
        if (! auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        // Build query same as exportAllProductsReport
        $query = Product::join('variations as v', 'products.id', '=', 'v.product_id')
            ->leftjoin('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
            ->leftjoin('categories as c1', 'products.category_id', '=', 'c1.id')
            ->leftjoin('categories as c2', 'products.sub_category_id', '=', 'c2.id')
        ...
        try {
            ...
            \Log::error('Update Products CSV Query Error: ' . $e->getMessage());
            return back()->with('error', 'Error fetching products: ' . $e->getMessage());
        }
        ...
        // Format data exactly like exportAllProductsReport
        foreach ($products as $product) {
            // Format product name
            if ($product->product_type == 'variable') {
                $product_name = $product->product_name . ' - ' . ($product->product_variation ?? '') . ' - ' . ($product->variation_name ?? '');
            ...
            if (!empty($product->sale_price) && $product->sale_price > 0 && $product->sale_price < $selling_price) {
                ...
                $selling_price_val = $product->sale_price;
            }
        ...
        // Create CSV file in public directory for direct web access
        $csv_filename = 'products_export_1.csv';
        $exports_dir = public_path('exports');
        ...
        $csv_full_path = $exports_dir . '/' . $csv_filename;
        
        // Create CSV file
        $csv_file = fopen($csv_full_path, 'w');
        
        if ($csv_file === false) {
            \Log::error("Failed to create CSV file at: {$csv_full_path}");
            return back()->with('error', 'Failed to create CSV file. Please check directory permissions.');
        }
        ...
        // Get public URL with cache-busting parameter for faster Google Sheets refresh
        $app_url = config('app.url');
        $timestamp = time(); // Current timestamp to force refresh
        $public_url = rtrim($app_url, '/') . '/reports/products-csv?v=' . $timestamp;
        
        \Log::info("Products CSV updated for Google Sheets - File path: {$csv_full_path}");
        \Log::info("Products CSV updated for Google Sheets - Public URL: {$public_url}");
        
        return back()->with('status', [
            'success' => 1,
            'msg' => 'Products CSV file updated successfully! Use this URL in Google Sheets (refreshes faster): ' . $public_url
        ]);
    }

    /**
     * Serve products CSV file with cache-busting for faster Google Sheets refresh
     * This route is publicly accessible (no auth required) for Google Sheets IMPORTDATA
     *
     * @return \Illuminate\Http\Response
     */
    public function serveProductsCsv(Request $request)
    {
        $csv_filename = 'products_export_1.csv';
        $csv_full_path = public_path('exports/' . $csv_filename);
        
        if (!file_exists($csv_full_path)) {
            abort(404, 'CSV file not found. Please update the CSV first.');
        }
        
        // Serve file with cache-busting headers to force Google Sheets refresh
        // These headers tell Google Sheets to always fetch fresh data
        return response()->file($csv_full_path, [
            'Content-Type' => 'text/csv; charset=UTF-8',
            'Cache-Control' => 'no-cache, no-store, must-revalidate, max-age=0',
            'Pragma' => 'no-cache',
            'Expires' => '0',
            'Last-Modified' => gmdate('D, d M Y H:i:s', filemtime($csv_full_path)) . ' GMT',
            'ETag' => md5_file($csv_full_path),
        ]);
    }
    ...
    public function getLotReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $products = $query->select(
                ...
                'sub_sku',
                'pl.lot_number',
                DB::raw('COALESCE(MAX(pl.battery_percentage), 0) as battery_percentage'),
            ...
            ->groupBy('pl.battery_percentage');

            // Apply stock status filter after grouping using HAVING clause
            $stock_status = $request->input('stock_status');
            if (!empty($stock_status)) {
                // Build the stock calculation expression exactly as in SELECT
                // The location_filter format should match what's used in SELECT
                $stock_calc = "( COALESCE((SELECT SUM(quantity - quantity_returned) from purchase_lines as pls " . $location_filter . " variation_id = v.id AND lot_number = pl.lot_number), 0) - SUM(COALESCE((tspl.quantity - tspl.qty_returned), 0)))";
                
                if ($stock_status == 'in_stock') {
                    // Filter for products with stock > 0
                    $products = $products->havingRaw($stock_calc . ' > 0');
            ...
            return Datatables::of($products)
                ...
                ->rawColumns(['exp_date', 'stock', 'total_sold', 'total_adjusted', 'battery_percentage'])
                ->make(true);
        }
    ...
    public function productSellReportBy(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            // For brand group_by, return product-level data
            if ($group_by == 'brand') {
                $query = TransactionSellLine::join(
                    'transactions as t',
                    'transaction_sell_lines.transaction_id',
                ...
                // Add per-location stock columns using subqueries to avoid duplication
                $location_ids_for_columns = $permitted_locations == 'all'
                    ? BusinessLocation::where('business_id', $business_id)->pluck('id')
                    : collect((array)$permitted_locations);

                foreach ($location_ids_for_columns as $locId) {
                    $query->addSelect(DB::raw("(SELECT COALESCE(SUM(vld3.qty_available), 0) FROM variation_location_details as vld3 WHERE vld3.variation_id = v.id AND vld3.location_id = {$locId}) as loc_{$locId}"));
                }
                ...
                $datatable = Datatables::of($query)
                    ->addColumn('product', function ($row) {
                        if ($row->product_type == 'variable') {
                            return $row->product_name . ' - ' . $row->product_variation . ' - ' . $row->variation_name;
                        }
                    ...
                    ->editColumn('sub_category_name', '{{$sub_category_name ?? ""}}')
                    ->editColumn('brand_name', '{{$brand_name ?? __("lang_v1.no_brand")}}')
                    ->editColumn('current_stock', function ($row) {
                        if ($row->current_stock) {
                            $val = (float)$row->current_stock;
                            if (is_int($val) || $val == floor($val)) {
                                return (string)(int)$val;
                            }
                            return number_format($val, 2, '.', '');
                    ...
                    ->addColumn('purchase_price', function ($row) {
                        return $this->transactionUtil->num_f($row->cost_price ?? 0, true);
                    })
                    ->addColumn('selling_price', function ($row) {
                        return $this->transactionUtil->num_f($row->selling_price ?? 0, true);
                    })
                    ->addColumn('sale_price', function ($row) {
                        if (!empty($row->sale_price) && $row->sale_price > 0 && $row->sale_price < $row->selling_price) {
                            return '<span class="text-danger">' . $this->transactionUtil->num_f($row->sale_price, true) . '</span>';
                        }
                        return '';
                    })
                    ->addColumn('discount', function ($row) {
                        if (!empty($row->sale_price) && $row->sale_price > 0 && $row->sale_price < $row->selling_price) {
                            $diff = $row->selling_price - $row->sale_price;
                            $percent = round($diff / $row->selling_price * 100);
                            return $this->transactionUtil->num_f($diff, true) . ' (' . $percent . '%)';
                        }
                        return '';
                    })
                    ->editColumn('total_qty_sold', function ($row) {
                        return '<span data-is_quantity="true" class="display_currency sell_qty" data-currency_symbol=false data-orig-value="'.(float) $row->total_qty_sold.'" data-unit="" >'.(float) $row->total_qty_sold.'</span>';
                    })
                    ->editColumn('subtotal', function ($row) {
                ...
                $raw_columns = ['current_stock', 'subtotal', 'total_qty_sold', 'category_name', 'sub_category_name', 'brand_name', 'sale_price', 'discount', 'product', 'sku'];
                foreach ($location_ids_for_columns as $locId) {
                    $raw_columns[] = 'loc_' . $locId;
                }
                ...
                // Original logic for category group_by
            $query = TransactionSellLine::join(
                'transactions as t',
                'transaction_sell_lines.transaction_id',
                '=',
