        ...
        if (! empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $purchase_lines[$purchase_line->variation_id][$k]['battery_percentage'] = $purchase_line->battery_percentage;
                    ...
                    if (array_key_exists($location_id, $locations)) {
                        foreach ($value as $vid => $purchase_lines_data) {
                            ...
                            foreach ($purchase_lines_data as $k => $pl) {
                                ...
                                if (! empty($pl['battery_percentage'])) {
                                    $battery_percentage = $this->productUtil->num_uf($pl['battery_percentage']);
                                }
                                ...
                                if (! is_null($purchase_line)) {
                                    ...
                                    $purchase_line->battery_percentage = $battery_percentage;
