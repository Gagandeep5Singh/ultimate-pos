...
class LabelsController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        // If no default is set, try to find "20x30 mm stickerts roll" or similar
        if (empty($default)) {
            $default = $barcode_settings->first(function ($barcode) {
                $name = strtolower($barcode->name);
                return strpos($name, '20x30') !== false || 
                       strpos($name, 'stickerts roll') !== false || 
                       strpos($name, 'stickers roll') !== false;
            });
        }
