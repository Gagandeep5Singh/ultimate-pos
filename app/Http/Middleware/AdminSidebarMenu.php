    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.update')) {
                            $sub->url(
                                action([\App\Http\Controllers\ProductController::class, 'updateProductDetails']),
                                __('lang_v1.update_product_details'),
                                ['icon' => '', 'active' => request()->segment(1) == 'products' && request()->segment(2) == 'update-product-details']
                            );
                        }
                        if (auth()->user()->can('product.view')) {
                            $sub->url(
            ...
            $menu->dropdown(
                ...
                function ($sub) {
                    $sub->url(
                        action([\App\Http\Controllers\KamalTecCustomerController::class, 'index']),
                        'Customers',
                        ['icon' => '', 'active' => request()->segment(1) == 'kamal-tec-customers']
                    );
                    ...
                    $sub->url(
                        ...
                        'KT Phone sale Report',
                        ['icon' => '', 'active' => request()->segment(1) == 'kamal-tec-sale-report']
                    );
                },
                ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M18 2a3 3 0 0 1 3 3v14a3 3 0 0 1 -3 3h-12a3 3 0 0 1 -3 -3v-14a3 3 0 0 1 3 -3h12z"></path>
                    <path d="M9 6l0 12"></path>
                    <path d="M15 6l0 12"></path>
                  </svg>']
            )->order(46);
            //Accounts dropdown
