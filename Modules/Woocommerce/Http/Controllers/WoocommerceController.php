...
class WoocommerceController extends Controller
{
    ...
    /**
     * Synchronizes only price and stock by SKU (matches existing WooCommerce products)
     *
     * @return Response
     */
    public function syncPriceAndStockBySku()
    {
        $notAllowed = $this->woocommerceUtil->notAllowedInDemo();
        if (! empty($notAllowed)) {
            return $notAllowed;
        ...
        try {
            ...
            $result = $this->woocommerceUtil->syncPriceAndStockBySku($business_id, $user_id);

            DB::commit();

            $msg = __('woocommerce::lang.price_stock_synced', [
                'updated' => $result['updated_count'],
                'not_found' => $result['not_found_count'],
            ]);

            if ($result['error_count'] > 0) {
                $msg .= '. ' . __('woocommerce::lang.errors_occurred', ['count' => $result['error_count']]);
            }
