...
class JobSheetController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            // FILTER for delivered jobsheets
if (!empty(request()->get('has_invoice')) && request()->get('has_invoice') == 'true') {
    $job_sheets->whereHas('invoices');
} elseif (!empty(request()->get('has_invoice')) && request()->get('has_invoice') == 'false') {
    $job_sheets->whereDoesntHave('invoices');
}
 else {
    ...
    if (!empty($row->delivery_date)) {
        ...
        $delivery = \Carbon\Carbon::parse($row->delivery_date);
        $diff = $now->diff($delivery);
        $minutesLeft = $delivery->diffInMinutes($now, false); // negative if overdue

        $time_string = ($diff->invert ? '-' : '') . 
            ($diff->days > 0 ? $diff->days . 'd ' : '') . 
            $diff->h . 'h ' . $diff->i . 'm';

        // ✅ Yellow if less than 1 hour left and still not overdue
        if (!$diff->invert && $diff->days == 0 && $diff->h == 0 && $diff->i <= 59) {
            return '<span class="label label-warning" style="font-weight:bold; color: black;">' . $time_string . '</span>';
        }

        // 🔴 Red for overdue
        if ($diff->invert) {
            return '<span class="label label-danger">' . $time_string . '</span>';
        }

        // 🟢 Green for anything else
        return '<span class="label label-success">' . $time_string . '</span>';
    }
    return '';
})
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if (auth()->user()->can('repair.create') && $row->status == 'Completed') {
    $html .= '<li>
        <a href="'.action([\App\Http\Controllers\SellPosController::class, 'create']).'?sub_type=repair&job_sheet_id='.$row->id.'" class="cursor-pointer"><i class="fas fa-plus-circle"></i> '.__('repair::lang.add_invoice').'
                ...
                <a href="#" class="cursor-pointer extend-time-btn" data-id="'.$row->id.'">
                    <i class="fa fa-clock"></i> '.__('Update Delivery Time').'
                </a>
              </li>';
    ...
    // Only show "+" icon if no invoice exists
    $add_invoice = '';
    if ($row->invoices->count() === 0 && auth()->user()->can('repair.create')) {
    ...
    return '<a href="#" class="job-summary-link" data-id="'.$row->id.'">'.$row->job_sheet_no.'</a>';
})
                ->removeColumn('id')
                ->rawColumns(['action', 'service_type', 'delivery_date', 'repair_no', 'status', 'estimated_cost', 'created_at','time_left','job_sheet_no'])
                ->make(true);
        }
    ...
    public function create()
    {
        ...
        $default_status = '';
        $default_pending_id = RepairStatus::where('name', 'Pending')
        ...
        return view('repair::job_sheet.create')
           ->with(compact('repair_statuses', 'device_models', 'brands', 'devices', 
                          'default_status', 'technecians', 'business_locations', 
                          'types', 'customer_groups', 'walk_in_customer', 
                          'repair_settings', 'default_pending_id'));
    }
...
{
    $job = \Modules\Repair\Entities\JobSheet::find($id);
    ...
    return response()->json([
        'expected_delivery_date' => $job->delivery_date,
        'update_note' => $job->comment_by_ss ?? '' // or use another note field
    ]);
}
...
{
    $request->validate([
        'job_id' => 'required|exists:repair_job_sheets,id',
        'due_date' => 'required|date',
        'note' => 'required|string',
    ]);

    $job = JobSheet::findOrFail($request->job_id);

    $old_due_date = $job->delivery_date;

    // ✅ Only update delivery_date — DO NOT touch comment_by_ss
    $job->delivery_date = $request->due_date;
    $job->save();

    // ✅ Log this action in activity only
    activity()
        ->performedOn($job)
        ->causedBy(auth()->user())
        ->withProperties([
            'note' => $request->note,
            'completed_on_from' => $old_due_date,
            'completed_on_to' => $request->due_date,
        ])
        ->log('delivery_time_extended');

    return back()->with('status', ['success' => true, 'msg' => 'Updated!']);
}
    ...
    public function summary($id)
    {
        ...
        $activities = Activity::forSubject($job)
            ->with('causer') // gets user who did it
            ->latest()
            ->get();

        // Get parts used
        $parts = $job->getPartsUsed();

        // Get repair settings for checklist
        $repair_settings = $this->repairUtil->getRepairSettings($business_id);
        $checklists = [];
        if (!empty($job->deviceModel) && !empty($job->deviceModel->repair_checklist)) {
            $checklists = explode('|', $job->deviceModel->repair_checklist);
        }
        ...
        return view('repair::job_sheet.partials.summary_modal', compact('job', 'activities', 'parts', 'checklists', 'repair_settings'));
    }
}
