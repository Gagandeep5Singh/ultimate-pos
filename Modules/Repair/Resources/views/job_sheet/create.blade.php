    ...
    {!! Form::open(['action' => '\Modules\Repair\Http\Controllers\JobSheetController@store', 'id' => 'job_sheet_form', 'method' => 'post', 'files' => true]) !!}
        ...
        <div class="row">
            @if(count($business_locations) == 1)
                @php 
                    $default_location_id = current(array_keys($business_locations->toArray()));
                @endphp
            @else
                @php 
                    $session_location_id = session('user.location_id');
                    $default_location_id = !empty($session_location_id) && isset($business_locations[$session_location_id]) ? $session_location_id : null;
                @endphp
            @endif
            <div class="col-md-3">
                <div class="form-group">
                    {!! Form::label('location_id', __('business.business_location') . ':*' )!!}
                    {!! Form::select('location_id', $business_locations, $default_location_id, ['class' => 'form-control', 'placeholder' => __('messages.please_select'), 'required', 'style' => 'width: 100%;']); !!}
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
